...
def plot_confusion_matrix(class_names, y_pred, y_test, title="Confusion Matrix"):
    """
    Function to compute a Confusion Matrix for the FFNN. Saves to a png.

    Parameters
    ----------
    class_names     : Either a make or miss
    y_pred          : Values predicted my the model
    y_test          : Ground truth values for FGM
    """
    ...
    fig.savefig('Python/ffnn_results/stage_2_confusion_matrix_5_fold.png')
    
def preprocess(dataset):
    '''
    Uses 12 handpicked features, standardizes continuous data and uses 5-fold cross validation on the data.

    Parameters
    ----------
    dataset     : The NFL kicker data found in data/preprocessed folder

    Returns
    ----------
    kfold_sets  : List of 5 folds containing training and testing splits of data and labels.
    '''
    data_cols = ["Home", "kickLength", "quarter", "scoreDifference", "secondsRemain", "average_temperature", "real_WindSpeed", "condition", "kickerId", "height", "weight"]
    cols_to_scale = ["kickLength", "scoreDifference", "secondsRemain", "average_temperature", "real_WindSpeed", "height", "weight"]
...
def plot_metrics(history):
    ...
    fig.tight_layout()
    fig.savefig('Python/ffnn_results/stage_2_metrics.png')
...
def get_clutch_scores(dataset, rows, scores, player_dict=player_dict):
    ...
    Returns
    ----------
    player_scores      : Dataframe of probabilities categorized by field goal distance and player
    player_scores_2    : Dataframe of probabilities categorized by score difference and player. (Positive means that the kicker's team is wining)
    '''
