...
def plot_confusion_matrix(class_names, y_pred, y_test, title="Confusion Matrix"):
    ...
    fig.savefig('Python/ffnn_results/stage_1_confusion_matrix_5_fold.png')
    
def preprocess(dataset):
    '''
    Uses 8 handpicked features, standardizes continuous data and uses 5-fold cross validation on the data.
...
def peek_majority_prediction(dataset):
    # Baseline Accuracy is about 83.5%. We need to do better than this!
...
def plot_metrics(history):
    ...
    fig.tight_layout()
    fig.savefig('Python/ffnn_results/stage_1_metrics.png')
...
def get_clutch_scores(dataset, rows, scores):
    ...
    Returns
    ----------
    player_agnostic_scores      : Dataframe of probabilities categorized by field goal distance
    player_agnostic_scores_2    : Dataframe of probabilities categorized by score difference. (Positive means that the kicker's team is wining)
    '''
